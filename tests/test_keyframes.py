...
class ServiceOwnerTests(unittest.TestCase):
    ...
    def test_relay_keyframe_uses_identity_board_and_previous_final_frame(self):
        seen = {}
        ...
        project = {"id": "p", "bible": {"style_anchor": "d" * 32}}
        shot = {"id": "s", "request": {"prompt": "Mina keeps walking"}, "title": "S"}
        with patch.object(backend, "submit_job", fake_submit), \
             patch.object(backend, "keyframe_wait", return_value={"status": "failed", "error": {"code": "stub"}}), \
             patch.object(backend, "FACTORY", unittest.mock.MagicMock()), \
             patch.object(backend.worker, "parse_request", lambda raw, pp: (raw, raw.get("external"), None)):
            with self.assertRaises(ValueError):
                backend.keyframe_generate(project, shot, backend.SERVICE_OWNER, "kf1", 1, "a" * 32,
                                          "1280x720", ["b" * 32, "c" * 32])
        self.assertEqual(seen["parameters"]["reference_2"], "b" * 32)
        self.assertEqual(seen["parameters"]["reference_3"], "c" * 32)
        self.assertNotIn("d" * 32, seen["parameters"].values())
