...
export type Asset = { id: string; name: string; url: string; kind: "image" | "video"; size_bytes: number; width: number; height: number };
...
export function MediaLibrary({ locale, onSelect }: { locale: keyof typeof fileCopy; onSelect: (asset: Asset) => void }) {
  ...
  return <section className="mb-6 border border-border bg-white p-5">
    <div className="flex flex-wrap items-center justify-between gap-4">
      ...
      <input ref={input} type="file" accept="image/png,image/jpeg,image/webp,video/mp4" className="hidden" aria-label={text.upload} onChange={(event) => { upload(event.target.files?.[0]); event.target.value = ""; }} />
    ...
    <p className="mt-3 text-[11px] leading-5 text-muted-foreground">{text.limit}</p>
    ...
    <div className="mt-5 grid gap-4 sm:grid-cols-2">
      {assets.map((asset) => <article key={asset.id} className="min-w-0 border border-border p-3">
        <div className="aspect-video bg-[#101211]">{asset.kind === "image" ? <img src={`${API_BASE}${asset.url}`} alt={asset.name} loading="lazy" className="h-full w-full object-contain" /> : <video src={`${API_BASE}${asset.url}`} controls preload="metadata" className="h-full w-full object-contain" />}</div>
        <p className="mt-3 truncate text-xs font-bold" title={asset.name}>{asset.name}</p>
        <p className="mt-1 text-[10px] text-muted-foreground">{asset.width} × {asset.height} · {(asset.size_bytes / 1024 / 1024).toFixed(2)} MiB</p>
