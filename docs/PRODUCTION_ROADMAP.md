...
十六張工單全部合併上線；七個階段都有頁面，七個服務常駐。尚未補跑的真素材／真模型／登入檢查集中在 `docs/PENDING_TESTS.md`，做完一項劃掉一項。
...
>
> 指定 nearest-rank 是因為原規格只寫「p90」沒說怎麼算，而算法之間差得夠多：エイサー 同一份殘差，
> nearest-rank 0.910 s、線性內插 0.923 s、exclusive 1.027 s。在舊的 1.0 s 門檻下這組數字**判定會翻面**；
> 新門檻 1.5 s 三種都過，但規格仍要指定算法，否則下一份素材同樣的歧義會再回來。
