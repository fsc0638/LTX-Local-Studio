...
| 項目 | 怎麼跑 | 預期 | 來源 |
|---|---|---|---|
| 後製放大／清理 | 05 對核准 take 放大 ×2、塗遮罩清理 | 新 take 過 QC；清理後 CJ 不低於清理前（要有臉的素材） | D3 —— 09-09 放大 ×2 實跑 187 s 後被技術檢查拒絕（48/49 幀）：`encode()` 的 `-shortest` 被 2.01 s 的音軌截掉一幀，已修（5cc2350，含回歸測試）；**ltx-post 仍跑舊碼，等核准重啟後再跑放大與清理** |
