...
```text
背景任務：【工單 <id> · 繼續】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin，切到 wo/<id 小寫>，讀 docs/work-orders/<id>.md 的進度區與最近三個 commit，從第一個未完成項接著做。規則、分支紀律、15 分鐘紀律與回報格式同「<id> · 執行」那一則；規格仍是 docs/PRODUCTION_ROADMAP.md「<id>」節。
```
...
開始前：git fetch origin；若分支 wo/a1 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/A1.md 的進度區（沒有就建立）。
硬規則：只在 wo/a1 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 A1 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/a1（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/a2 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/A2.md 的進度區（沒有就建立）。
硬規則：只在 wo/a2 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 A2 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/a2（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/b0 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/B0.md 的進度區（沒有就建立）。
硬規則：只在 wo/b0 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 B0 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/b0（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/b1 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/B1.md 的進度區（沒有就建立）。
硬規則：只在 wo/b1 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 B1 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/b1（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/b2 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/B2.md 的進度區（沒有就建立）。
硬規則：只在 wo/b2 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 B2 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/b2（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/b3 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/B3.md 的進度區（沒有就建立）。
硬規則：只在 wo/b3 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 B3 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/b3（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/b4 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/B4.md 的進度區（沒有就建立）。
硬規則：只在 wo/b4 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 B4 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/b4（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/c1 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/C1.md 的進度區（沒有就建立）。
硬規則：只在 wo/c1 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 C1 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/c1（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/c2 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/C2.md 的進度區（沒有就建立）。
硬規則：只在 wo/c2 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 C2 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/c2（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/c3 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/C3.md 的進度區（沒有就建立）。
硬規則：只在 wo/c3 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 C3 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/c3（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/c4 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/C4.md 的進度區（沒有就建立）。
硬規則：只在 wo/c4 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 C4 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/c4（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/d1 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/D1.md 的進度區（沒有就建立）。
硬規則：只在 wo/d1 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 D1 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/d1（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/d2 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/D2.md 的進度區（沒有就建立）。
硬規則：只在 wo/d2 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 D2 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/d2（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/d3 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/D3.md 的進度區（沒有就建立）。
硬規則：只在 wo/d3 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 D3 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/d3（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/d4 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/D4.md 的進度區（沒有就建立）。
硬規則：只在 wo/d4 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 D4 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/d4（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
...
開始前：git fetch origin；若分支 wo/d5 不存在，從 origin/main 建立；存在就切過去並 rebase 到 origin/main。讀 docs/work-orders/D5.md 的進度區（沒有就建立）。
硬規則：只在 wo/d5 分支 commit 與 push，絕對不 push、merge 或 rebase 到 main（main 一推就自動部署正式站）；不重啟 ltx-web、ltx-api；不動 /opt/studio 的權重；系統層級動作（apt、systemctl、寫 /etc 或 unit 檔啟用）一律先發核准區塊等阿寶回覆，需要 sudo 的指令列給阿寶自己跑。
這一則的工作：
...
```text
背景任務：【工單 D5 · 驗收】
專案目錄 "/home/kwayrdc/LTX Local Studio"。git fetch origin 後切到分支 wo/d5（不存在就回報並停止）。這是驗收不是開發：不修改程式與文件；只允許為了讓測試跑起來準備測試資料庫或暫存目錄，且做完要清掉。
