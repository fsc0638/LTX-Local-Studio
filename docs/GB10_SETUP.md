...
| エイサーの足音 | **0.910** | 0.923 | **1.027** |

エイサー 那一格三種算法橫跨 0.910–1.027 s，正好騎在當時的 1.0 s 門檻上。**規格因此必須指定算法**；
現在指定 nearest-rank，寫成 `ceil(0.9n)-1`。

實作這個公式要小心：`-(-int(0.9*n)//1)` 看起來是 ceil，其實 `int()` 已經先截斷，整串等於 `int(0.9n)`。
兩者只在 `0.9n` 剛好是整數時差一格 —— n=40 正是這種情形（索引 35 對 36）。上表 エイサー 一格
曾因此誤記為 1.040，2026-09-06 驗收量到 0.910 才發現；**驗收是對的**。

**VAD 試過，更差，不要再試。** stable-ts 的 `vad=True` 理論上有助於長器樂空白，實測是災難：

```
現況    三線 1.030   結い 0.500   エイサー 0.910
...
| エイサーの足音 (n=40) | 0.39s / 0.7拍 | 0.79s | 0.91s / 1.6拍 | 1.29s | 23/40 | 36/40 | 40/40 |

**p50 全部在一拍以內**：典型的行對得相當準，這是對時本身的品質。
**p90 落在 0.9–1.8 拍**，而它的尾巴由少數 LRC 本身不準的行主導 —— 用它單獨當硬門檻，量到的是歌詞檔
