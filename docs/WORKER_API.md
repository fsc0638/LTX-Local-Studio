...
目前程式契約版本：**1.2.0**。固定路徑 `/api/v1/*`；不建立任何特定專案專用通道。
...
本機安全上限仍為 257 幀；不是 LTX 全系列的絕對極限，也不是整支 MV 的上限。
...
例如 10秒／24FPS → 241幀 → 約10.042秒；10秒／30FPS 超過257幀，回400，**不偷偷改成8.57秒**。

原有網頁秒數選單只列到10秒，且切 FPS 時會用 `min(257, ...)` 壓回上限，這能解釋部分縮短。
這次沒有修改原網頁；新的 v1 API 不做靜默截短。
...
本機20秒／24FPS 可作下一階段481幀實驗，但此版本尚未放寬上限、也沒有20秒壓力測試。
[LTX-2.3 官方 API 規格](https://docs.ltx.io/models/ltx-2-3)

整支30秒、60秒或數分鐘 MV 由上游分鏡、逐鏡生成及剪輯，長度不受單次257幀限制；不代表可一次生成同長度連續鏡頭。
