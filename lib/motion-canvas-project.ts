...
if ! command -v node >/dev/null 2>&1 || ! command -v npm >/dev/null 2>&1; then
  osascript -e 'display alert "需要 Node.js" message "請先安裝 Node.js 22 或 24，再重新執行此檔案。"'
  exit 1
fi
...
export function buildMotionCanvasReadme(headline: string) {
  return `# ${headline}

## macOS 一鍵啟動

因為這是下載的指令檔，macOS 第一次開啟時請不要直接雙擊：

1. 按住 Control 點擊（或右鍵點擊）\`start-mac.command\`。
2. 選擇「打開」，再於確認視窗按「打開」。
3. 它會在需要時執行 \`npm install\`、啟動 Editor，並開啟 http://127.0.0.1:9000/。

如果視窗只有「完成」與「丟到垃圾桶」，請到「系統設定 → 隱私權與安全性」，在安全性區段按「仍要打開」，再回來重試。

### Terminal 備援

在專案資料夾開啟 Terminal，執行：

\`\`\`bash
bash ./start-mac.command
\`\`\`

不建議移除整包檔案的 Gatekeeper 隔離標記。
